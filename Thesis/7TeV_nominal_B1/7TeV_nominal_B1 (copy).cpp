...
// Initialise the random number generator with the seed
	...
	string input_dir = "/UserSim/data/7TeV_nominal_B1/";
	
	//~ string output_dir = "/test2/UserSim/outputs/HL/";
	string output_dir = "/Build/UserSim/outputs/7TeV_nominal_B1/";
...
///////////////////////////////////
	...
	//std::vector<YCor*> TestStuff;
	//myAccModel->ExtractTypedElements(TestStuff,"MCBWV.5R3.B2");
	//std::cout << "Extrtacted " << TestStuff.size() << " elements" << std::endl;
	//std::cout << TestStuff[0]->GetEMField() << std::endl;
...
//new: (setting number of points to 0 defaults to using the step size 0.1m)
  ApertureSurvey* myApertureSurvey = new ApertureSurvey(myAccModel, full_output_dir, 0.1, 0);

//this also works for 10cm bins:
 // ApertureSurvey* myApertureSurvey = new ApertureSurvey(myAccModel, full_output_dir);
...
/////////////////////
    ...
    string t_o_f = full_output_dir+tof;
     ...
     //~ OUTPUT AV
     //uncomment the followin 8 lines to get output file

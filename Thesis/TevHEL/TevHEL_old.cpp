...
int main(int argc, char* argv[])
{
	...
	//~ string output_dir = "/HEL_RUNS/UserSim/outputs/LHC_HEL/";	
	string output_dir = "/Build/Thesis/outputs/TevHEL/";
	//~ string output_dir = "/HEL_TESTS/UserSim/outputs/LHC_HEL/";		
	...
	bool cut_distn					= 0;		//cut out stable part of distn
...
///////////////////////////////////
	...
	MADInterface* myMADinterface = new MADInterface( directory+input_dir+"twiss.hel.7.0tev.b1.tfs", beam_energy );
...
///////////////////////
    ...
    // Nominal -0.2 sigma = -5.35E-5
    // HLv1.2  -0.2 sigma = -5.328E-5
    
    //~ collimator_db->SelectImpactFactor(start_element, 1.0e-6);
    //~ collimator_db->SelectImpactFactor(start_element, -5.35E-5);
    collimator_db->SelectImpactFactor(start_element, -5.35E-4);
...
/////////////////	
	...
	if(hel_on){
		...
		if(LHC_HEL){	// LHC: 3m, 10KeV, 5A
			myHELProcess = new HollowELensProcess(0, 1, 5, 0.195, 2.334948339E4, 3.0);
		}
		else{			//Tevatron: 2m, 5KeV, 1.2A
			myHELProcess = new HollowELensProcess(0, 1, 1.2, 0.138874007, 2.334948339E4, 2.0);
		}
...
double Tunex(0.), Tuney(0.), dTunex(0.), dTuney(0.);
	...
	if(calc_tune){
		...
		//~ for(ParticleBunch::iterator tuneparticle = myBunch->begin(); tuneparticle != myBunch->end(); ++tuneparticle){
		//~ ++tuneparticle;
		//~ ++tuneparticle; ++tuneparticle; ++tuneparticle; ++tuneparticle; ++tuneparticle;
		//~ ParticleBunch::iterator tuneparticle = myBunch->begin();
		//~ tuneparticle++;			
	...
	if(tune_test){		
		...
		#endif
		{
			...
			//~ HELProcess->SetRadiiSigma(4, 6.8, model, emittance, emittance, twiss_no_error);	
			//~ HELProcess->SetOpMode(DC);
			
			//void BetatronTunes::FindTunes(PSvector& particle, int ntrack, bool diffusion)
			//~ BetatronTunes* tune = new BetatronTunes(myAccModel, beam_energy);
			//~ tune->SetHELProcess(myHELProcess);
				
			//~ for(ParticleBunch::iterator tunepart = (myBunch->begin())+9; tunepart != myBunch->end(); ++tunepart){
			//~ for(PSvectorArray::iterator tunepart = (myBunch->begin())+1; tunepart != myBunch->end(); ++tunepart){
				...
				//~ it += step;								
			//~ }			
			//~ for(PSvectorArray::iterator tp = ((myBunch->begin())+1); tp != myBunch->end(); tp++){
			for(PSvectorArray::iterator tp = myBunch->begin(); tp != myBunch->end(); tp++){
				
				BetatronTunes* tune = new BetatronTunes(myAccModel, beam_energy);
				//~ tune->SetHELProcess(myHELProcess);
				
				//FindTunes(particle, n turns, diffusion (n turns *2))
				tune->FindTunes((*tp), powertwo, 0);
				*tune_output << it << "\t" << (*tp).x()/1.61E-5 << "\t" << tune->GetQx() << endl;
				cout << " \n\t" << it << "\t" << "Tunex = " << tune->GetQx() << endl;

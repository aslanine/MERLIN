...
int main(int argc, char* argv[])
{
	...
	start_element = "IPA";// HORIZONTAL COLLIMATOR (x)
	...
	input_dir 	= "/FCC/Input/";
	output_dir 	= "/Build/FCC/outputs/IPA_LSS/";

		
	string batch_directory="29JulyTest/";
...
************************************/
	...
	MADInterface* myMADinterface;
		myMADinterface = new MADInterface( directory+input_dir+"IPA_R.tfs", beam_energy );
...
************/
	...
	// WONT WORK FOR NON PERIODIC LATTICE
    //~ int start_element_number = myAccModel->FindElementLatticePosition(start_element.c_str());
    
    //~ cout << "Found start element IPA at element number " << start_element_number << endl;
...
/*
	...
	collimator_db = new CollimatorDatabase( directory+input_dir+"Collimator_7TeV_pure.txt", myMaterialDatabase,  true);
...
************/
    ...
    if(input_distn){
		ifstream* bunch_input = new ifstream("/home/HR/Downloads/MERLIN_HRThesis/MERLIN/FCC/Input/3m_input_dist.dat");		
...
************************/
	...
	AcceleratorModel::Beamline beamline = myAccModel->GetBeamline();

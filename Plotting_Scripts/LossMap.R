...
#~ 	}
	infile<-read.table(paste("HLdustbin_losses.txt", sep=""))
#~ 	sort0<-0
	final <- rbind(infile)
#~ 	sort<-rbind(sort,sort0)
...
#plot coll losses in black
	plot(coll[,2], coll[,4], log='y', type='h', lwd=linewidth, lend=1, main = "IR7", xlab="s[m]", ylab = expression(paste("Inefficiency ", eta, " [", m^-1, "]")), xlim = xlimits, yaxt='n', panel.first=grid(equilogs=FALSE, lwd = linewidth/4 ), col = 'black' )	

...
#Open input file(s)
	...
	in1<-read.table("1ct.txt")
	in2<-read.table("2ct.txt")
	in3<-read.table("3ct.txt")
	in4<-read.table("4ct.txt")
...
#2D plot det vs sigma_x 
    gridcex <-0.25
#~     
    plot(ct1, detotm1, log='y', type = "p", lty=1, ylim=c(1E-10,1E-4), cex.lab=multi, cex.axis = multi, xlab="ct", ylab="|1-Det(OTM)|", pch ='x', col = col1) 
...
#~ 	abline(v=10, lty=2, lwd=gridcex, col='gray80')
	...
	points(ct2, detotm2, log='y', type = "p", pch = 'x', col = col2)   
	points(ct3, detotm3, log='y', type = "p", pch = 'x', col = col3)   
	points(ct4, detotm4, log='y', type = "p", pch = 'x', col = col4)   

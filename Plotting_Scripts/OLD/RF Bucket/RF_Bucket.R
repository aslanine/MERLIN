...
#~ 	yp <- cbind(yp1,yp2,yp3,yp4,yp5,yp6,yp7,yp8,yp9,yp10)
	ct <- cbind(ct1,ct2,ct3,ct4,ct5,ct6,ct7,ct8,ct9,ct10)
	dp <- cbind(dp1,dp2,dp3,dp4,dp5,dp6,dp7,dp8,dp9,dp10)
...
#~     plot(x[,nofiles], xp[,nofiles], type = "p", main="x, x'", xlab="x", ylab="x'", pch ='.', col = colfunc12(turn))
    ...
    plot(ct[,10], dp[,10], type = "p", xlim=c(-0.5,1.5), ylim=c(-0.005,0.005), cex.lab=multi, cex.axis = multi, xlab="ct", ylab="dp", pch ='.', col = col10)

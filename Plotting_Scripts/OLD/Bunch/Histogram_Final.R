...
#Open input file(s)
	...
	for(i in 0:fileno){
		infile<-read.table(paste("Node_",i,"_final_bunch.txt", sep=""))
		xin<-0
		xin<-0
		yin<-0		
		ypin<-0
		...
		xin<-(as.numeric(as.character(infile$V1)))/0.000267067
...
#This controls plot pallete layout: 2 rows, 3 cols, margins of left, right, top, bottom
   ...
   # hist(x,5)
    multi<-2
   hist(x, 100, freq=FALSE, xlim=c(-6,6), ylim=c(0,1), main=" x hist", xlab= expression("Amplitude",sigma[x]), ylab=expression("dN/dN"[0]), pch ="x", col ='white', border='red', lwd=0.5, cex.lab=multi, cex.axis = multi)
#X,Y
    #plot(x, y, type = "p", main=" x, y", xlab="x", ylab="y", pch ="x", col ='red')
#Px,Py
    #plot(xp, yp, type = "p", main=" x', y'", xlab="x'", ylab="x'", pch ="x", col ='orange')
#X,Px
    #plot(x, xp, type = "p", main="x, x'", xlab="x", ylab="x'", pch ="x", col ='blue')
#Y,Py
    #plot(y, yp, type = "p", main="y, y'", xlab="y", ylab="y'", pch ="x", col ='green')
#ct,dp
   # plot(ct, dp, type = "p", main="ct, dp", xlab="ct", ylab="dp", pch ="x", col ='purple')
